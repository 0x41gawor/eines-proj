...
def measure_delay_component(received_time, sent_time):
    return 0.5*(received_time - sent_time)

def handle_probe_packetIn(packet, received_time, OWDs1_ctrl, other_s_OWD, switch_name):
...
def send_probe_packet(destination_switch, start_time, dpid):
...
def send_stats_request_packet(src_dpid, start_time):
